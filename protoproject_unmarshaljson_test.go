...
func TestProtoProjectUnmarshalJSON(t *testing.T) {
	...
	tests := []struct {
		...
		// 5: JSON object → Project with all fields including schema type
		{
	...
	for testNumber, test := range tests {
		...
		var actual ProtoProject
		err := jsonld.UnmarshalJSONStringOrJSONObject[ProtoProject, ProjectID, AnyProject](test.JSON, &actual)
